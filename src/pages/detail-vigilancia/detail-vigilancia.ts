...
/**
 * Generated class for the DetailVigilanciaPage page.
 *
...
@Component({
  selector: 'page-detail-vigilancia',
  templateUrl: 'detail-vigilancia.html',
})
export class DetailVigilanciaPage {
...
firedata = firebase.database().ref('/users');
  ...
  constructor(public afireauth: AngularFireAuth,
    ...
    if(this.id != 0){
         notesService.getNoteVigilancia(this.id)
         .valueChanges()
         .subscribe(note => {
          this.note = <any>note;
         });
  ...
  addNoteVigilancia(){
    if(this.id != 0){
        this.notesService.editNoteVigilancia(this.note);
        alert('Nota editada con exito');
      
    }else{
      this.note.id = Date.now();
      this.notesService.createNoteVigilancia(this.note);
  ...
  deleteNoteVigilancia(){
    this.show =false;
    this.notesService.deleteNoteVigilancia(this.note);
