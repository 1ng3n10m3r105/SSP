...
/**
 * Generated class for the DetailAlcoholimetroPage page.
 *
...
@Component({
  selector: 'page-detail-alcoholimetro',
  templateUrl: 'detail-alcoholimetro.html',
})
export class DetailAlcoholimetroPage {
...
firedata = firebase.database().ref('/users');
  ...
  constructor(public afireauth: AngularFireAuth,
    ...
    if(this.id != 0){
         notesService.getNoteAlcoholimetro(this.id)
         .valueChanges()
         .subscribe(note => {
          this.note = <any>note;
         });
  ...
  addNoteAlcoholimetro(){
    if(this.id != 0){
      ...
      this.notesService.createNoteAlcoholimetro(this.note);
  ...
  deleteNoteAlcoholimetro(){
    this.show =false;
    this.notesService.deleteNoteAlcoholimetro(this.note);
